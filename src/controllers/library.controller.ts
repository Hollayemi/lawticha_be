...
export const createOrderHandler = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    return (res as AppResponse).data({ order }, 'Order created successfully');
  }
);
...
export const adminCreateBookHandler = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const getCoverUrlFromFile = coverUrl ||  (await CloudinaryService.uploadImage(coverFile, "books/covers", 'image')).url
    const getPDFUrlFromFile = pdfUrl || (await CloudinaryService.uploadImage(pdfFile, "books/pdfs", 'raw')).url
