...
export async function bookConsultation(citizenId: string, input: BookConsultationInput) {
  ...
  // Generate receipt ID
  const receiptId = `RCP-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
...
export async function requestLawyerMatch(citizenId: string, input: RequestMatchInput) {
  ...
  const request = await LawyerRequestModel.create({
    ...
    urgency: urgencyMap[input.urgency] || input.urgency,
    location: input.location,
    budget: input.budgetRange,
    description: input.description,
    status: 'pending',
    timeline: [
      { time: new Date(), label: 'Request submitted', note: 'Awaiting lawyer match' },
    ],
  });

  // This would trigger a matching algorithm in a real implementation
  // For now, just return the request

  return {
    requestId: request._id,
    status: 'pending',
    estimatedMatchTime: '2 hours',
  };
}
