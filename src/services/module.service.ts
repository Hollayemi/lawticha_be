...
function toModuleDto(doc: IModule & { _id: Types.ObjectId }) {
...
function toTopicDto(doc: ITopic & { _id: Types.ObjectId }) {
  const obj = (doc as any).toObject ? (doc as any).toObject() : doc;
  return {
    id:             String(obj._id),
...
function toSubTopicDto(doc: ISubTopic & { _id: Types.ObjectId }) {
  const obj = (doc as any).toObject ? (doc as any).toObject() : doc;
  return {
    id:              String(obj._id),
...
function toCommentDto(doc: any, replies: any[] = []):any {
...
// Derive random pastel-ish colour from a string (for auto-assigned instructor colours)
